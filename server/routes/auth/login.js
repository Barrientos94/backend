...
const UsuarioModel = require('../../models/usuario/usuario.model'); //buscamos el modelo de usuario
...
app.post('/login', async(req,res)=>{
   ...
   if(!strEmail || !strContrasena)
   {
        return res.status(400).json({
            ok:false,
            msg: !strEmail && !strContrasena ? 'No se recibio un strEmail y  strContrasena, favor de ingresarlos' : !strEmail ? 'No se recibio el strEmail, favor de ingresarlo' : 'No se recibio el strContrsena, favor de ingresarlo',
            cont: {
                strEmail,
                strContrasena
            }
...
if(!encontroEmail)
{
    return res.status(400).json({
        ok:false,
        msg:'El correo* o la contraseña son incorrectas favor de verificarlo',
        cont:{
            strEmail,
            strContrasena
        }
